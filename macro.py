...
class MacroRecorder:
    ...
    def toggle_replaying(self, update_ui, repetitions=1):
        if self.replaying:
            self.replaying = False
            ...
            if not self.events:
                ...
                return
            self.replaying = True
            threading.Thread(target=self.replay_events, args=(update_ui, repetitions), daemon=True).start()
            update_ui(True, "Stop Replay")

    def replay_events(self, update_ui, repetitions):
        ...
        for rep in range(repetitions):
            ...
            for event in self.events:
                ...
                delay = current_offset - previous_offset
                if delay > 0:
                    precise_sleep(delay)
...
def build_ui():
    ...
    recording_button = tk.Button(
        ...
        command=lambda: threading.Thread(
            target=recorder.toggle_recording, args=(update_recording_status,), daemon=True
        ).start()
    )
    ...
    replay_button = tk.Button(
        ...
        command=lambda: threading.Thread(
            target=recorder.toggle_replaying, args=(update_replay_status, get_replay_count()), daemon=True
        ).start()
    )
